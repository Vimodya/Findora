...
/// <summary>
/// End-to-end tests for Module 4 (Lost Item Reporting) against the real
/// HTTP pipeline — same approach as <c>UsersEndpointsTests</c>.
/// </summary>
public class LostItemsEndpointsTests : IClassFixture<CustomWebApplicationFactory>
{
    private const string Password = "P@ssword123";

    // Same rationale as UsersEndpointsTests — the server serializes enums
    // as strings (Program.cs's JsonStringEnumConverter), so the test
    ...
    public LostItemsEndpointsTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    public async Task Create_WhenAuthenticated_ReturnsCreatedWithReport()
    {
        ...
        var response = await authedClient.PostAsJsonAsync("/api/v1/lost-items", ValidCreatePayload(categoryId));

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        var report = await response.Content.ReadFromJsonAsync<LostItemResponse>(JsonOptions);
        Assert.NotNull(report);
        Assert.Equal(userId, report!.UserId);
        Assert.Equal(categoryId, report.CategoryId);
        Assert.Equal(LostItemStatus.Active, report.Status);
    ...
    public async Task Create_WithoutToken_ReturnsUnauthorized()
    {
        ...
        var response = await _client.PostAsJsonAsync("/api/v1/lost-items", ValidCreatePayload(categoryId));
    ...
    public async Task Create_WithInvalidCategory_ReturnsBadRequest()
    {
        ...
        var response = await authedClient.PostAsJsonAsync("/api/v1/lost-items", ValidCreatePayload(Guid.NewGuid()));
    ...
    public async Task Create_WithMissingRequiredFields_ReturnsBadRequest()
    {
        ...
        var response = await authedClient.PostAsJsonAsync("/api/v1/lost-items", new
        {
            title = "",
            description = "",
    ...
    [Fact]
    public async Task Create_WithFutureDateLost_ReturnsBadRequest()
    {
        var (accessToken, _, _) = await RegisterAndLoginAsync();
        var categoryId = await GetFirstCategoryIdAsync();
        using var authedClient = AuthedClient(accessToken);
        ...
        payload.DateLost = DateTime.UtcNow.AddDays(5);

        var response = await authedClient.PostAsJsonAsync("/api/v1/lost-items", payload);
    ...
    public async Task Create_ServerAssignsAuthenticatedUsersId_IgnoringClientSuppliedUserId()
    {
        ...
        // CreateLostItemRequest has no UserId property at all, so this
    ...
    public async Task GetById_WhenOwner_ReturnsReport()
    {
        ...
        var response = await authedClient.GetAsync($"/api/v1/lost-items/{created.Id}");
    ...
    public async Task GetById_ForNonexistentReport_ReturnsNotFound()
    {
        ...
        var response = await authedClient.GetAsync($"/api/v1/lost-items/{Guid.NewGuid()}");
    ...
    public async Task GetById_ForAnotherUsersReport_ReturnsForbidden()
    {
        ...
        var response = await otherClient.GetAsync($"/api/v1/lost-items/{created.Id}");
    ...
    public async Task GetMy_ReturnsOnlyOwnReports_Paginated()
    {
        ...
        var response = await clientA.GetAsync("/api/v1/lost-items/my?page=1&pageSize=1");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var page = await response.Content.ReadFromJsonAsync<PaginatedLostItemsResponse>(JsonOptions);
        Assert.NotNull(page);
    ...
    public async Task Update_WhenOwner_PersistsAllowedFields()
    {
        ...
        var response = await authedClient.PutAsJsonAsync($"/api/v1/lost-items/{created.Id}", new
        {
            title = "Updated title",
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var updated = await response.Content.ReadFromJsonAsync<LostItemResponse>(JsonOptions);
    ...
    public async Task Update_CannotModifyProtectedFields()
    {
        ...
        // UpdateLostItemRequest has no Id/UserId/CreatedAt/UpdatedAt/
        ...
        var payload = new Dictionary<string, object?>
        {
            ...
            ["categoryId"] = categoryId,
            ["dateLost"] = created.DateLost,
            ["id"] = Guid.NewGuid().ToString(),
            ["userId"] = Guid.NewGuid().ToString(),
            ["status"] = "Resolved"
        };

        var response = await authedClient.PutAsJsonAsync($"/api/v1/lost-items/{created.Id}", payload);

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var updated = await response.Content.ReadFromJsonAsync<LostItemResponse>(JsonOptions);
        Assert.Equal(created.Id, updated!.Id);
        Assert.Equal(userId, updated.UserId);
        Assert.Equal(LostItemStatus.Active, updated.Status);
    }
    ...
    public async Task Update_ForAnotherUsersReport_ReturnsForbidden()
    {
        ...
        var response = await otherClient.PutAsJsonAsync($"/api/v1/lost-items/{created.Id}", new
        {
            title = "Hijacked",
            description = created.Description,
            categoryId,
    ...
    public async Task Cancel_WhenOwner_SoftCancelsReport()
    {
        ...
        var response = await authedClient.DeleteAsync($"/api/v1/lost-items/{created.Id}");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var cancelled = await response.Content.ReadFromJsonAsync<LostItemResponse>(JsonOptions);
        Assert.Equal(LostItemStatus.Cancelled, cancelled!.Status);
        ...
        var refetch = await authedClient.GetAsync($"/api/v1/lost-items/{created.Id}");
        ...
        var row = await db.LostItemReports.FirstOrDefaultAsync(r => r.Id == created.Id);
        Assert.NotNull(row);
        Assert.True(row!.IsDeleted);
        Assert.Equal(LostItemStatus.Cancelled, row.Status);
    }
    ...
    public async Task Cancel_ForAnotherUsersReport_ReturnsForbidden()
    {
        ...
        var response = await otherClient.DeleteAsync($"/api/v1/lost-items/{created.Id}");

        Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
    }

    [Fact]
    public async Task SuspendedUser_CannotCreateOrAccessLostReports()
    {
        var categoryId = await GetFirstCategoryIdAsync();
        ...
        var createAttempt = await userClient.PostAsJsonAsync("/api/v1/lost-items", ValidCreatePayload(categoryId));
        Assert.Equal(HttpStatusCode.Forbidden, createAttempt.StatusCode);

        var getAttempt = await userClient.GetAsync($"/api/v1/lost-items/{created.Id}");
    ...
    private async Task<LostItemResponse> CreateReportAsync(HttpClient client, Guid categoryId)
    {
        var response = await client.PostAsJsonAsync("/api/v1/lost-items", ValidCreatePayload(categoryId));
        response.EnsureSuccessStatusCode();
        return (await response.Content.ReadFromJsonAsync<LostItemResponse>(JsonOptions))!;
    }

    private static CreatePayload ValidCreatePayload(Guid categoryId) => new()
    {
        Title = "Lost backpack",
        Description = "Navy blue backpack with a laptop inside.",
        CategoryId = categoryId,
        DateLost = DateTime.UtcNow.AddDays(-1),
        Brand = "Northface",
        Color = "Navy"
    ...
    /// <summary>Mirrors <c>UsersEndpointsTests.PromoteToAdminAsync</c> — reaches into the test database directly since there's no self-service way to suspend an account.</summary>
